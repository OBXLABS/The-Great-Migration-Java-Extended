...
boolean PQLABS_TOUCHSCREEN = false;    //true when using PQLabs touchscreen
...
int[] FONT_SIZES = {14, 26, 56, 120};               //victor - collection of font sizes for differently sized beasts
...
int[] BEAST_RADIA = {100, 200, 300, 400};          //radia of the beasts' lens effect for differently sized beasts
...
final int NOISE_MIN_BEASTS = 10;                   //minimum number of beasts for noise calculation
final int NOISE_MAX_BEASTS = 24;                   //maximum number of beasts for noise calculation
final float NOISE_PERIOD_BEASTS = 500f;            //period noise calculation (bigger the number, the slower the change in maximum beasts)
final int MAX_BEASTS = 48;                         //maximum number of beasts
...
void setup() {
  ...
  //load font
  //victor - load largest size
  font = createFont(FONT_FILE, FONT_SIZES[3], true);
...
void readBeast(int id, int x, int y) {
  ...
  if (closestBeast != -1) {
    ...
    //victor - use internal target radius
    beasts[closestBeast].setTargetRadius(beasts[closestBeast].getDefaultTargetRadius());
  }
...
void drawSpray() {
  synchronized(sprayTokens) {
    ...
    while (it.hasNext()) {
      ks = (KineticString)it.next();
      //victor - use font size saved in kineticstring
      textFont(font, ks.fontSize);
...
//create a beast
Beast createBeast() {
  //victor - pick random size from 4 options, use in subsequent variable assigment regarding size of beast (default radius, default target radius, font size)
  int sizeIndex = (int)random(0, 4);
  
  //set default properties
  Beast beast = new Beast(getBeastStartPosition());
  beast.id = totalBeastCount;
  beast.setMagnification(0.85, random(0, TWO_PI));
  beast.setDefaultRadius(BEAST_RADIA[sizeIndex]);
  beast.setDefaultTargetRadius(BEAST_RADIA[sizeIndex] / 1.4);
  beast.setRadius(BEAST_RADIA[sizeIndex]);
  beast.setSpeed(random(0.5f, 4f));
  beast.setScatter(scatter);
  beast.shadow = shadow;
  beast.setForward(current.x > 0 ? PI : 0);
  beast.fontSize = FONT_SIZES[sizeIndex] * 2;
  if (beast.fontSize > FONT_SIZES[3]) beast.fontSize = FONT_SIZES[3];

  //add text to the book
  textAlign(LEFT, CENTER);
  textFont(font, FONT_SIZES[sizeIndex]);
...
PVector getBeastStartPosition() { 
  return getBeastStartPosition(MIN_BEAST_Z + random(MAX_BEAST_Z-MIN_BEAST_Z));
}
...
void mouseReleased(int id, int x, int y) {
  ...
  beast.scatter();
  //victor - use internal radius
  beast.setTargetRadius(beast.getDefaultRadius());
