...
public class Beast {
  ...
  float defaultRadius;      //victor - default lens radius
  float defaultTargetRadius;//victor - default lens radius when targeted
  ...
  //victor - fontsize and wiggle variables
  int fontSize;
  float wiggleAngle = 0;
  float wiggleSpeed = 0.01;
  float wiggleAmount = 25;
  ...
  //victor - make tail bend factor editable
  float tailBendFactor = 250;         //bend factor for the tail behavior
  ...
  //set/get radius
  //victor - get/set for default radius and default target radius
  void setRadius(float r) { radius = targetRadius = r; }
  void setDefaultRadius(float r) { 
    defaultRadius = r;
    //victor - set tail bend to accomodate size
    tailBendFactor = r;
    //victor - set target radious to proportional to default size
    targetRadius = r / 2;
  }
  void setDefaultTargetRadius(float r) { defaultTargetRadius = r; };
  void setTargetRadius(float r) { targetRadius = r; }
  float getDefaultRadius() { return defaultRadius; }
  float getDefaultTargetRadius() { return defaultTargetRadius; }
    ...
    for(int i = 0; i < origText.length; i++) {
      ...
      for(int j = 0; j < origData.types.length; j++) {
        ...
        for(int k = (j==0?0:origData.ends[j-1]); k < origData.ends[j]; k++) {
          ...
          // mouth delta
          //victor - move lens distortion origin from left to right
          mouthDelta.x = mouth.x - pOrig[0] + (wiggleAmount / 2) + (sin(wiggleAngle) * wiggleAmount);
  ...
  KineticString spray(float pushAngle, float pushForce, boolean override) {
    ...
    //add a new token
    // victor - add fontSize to spray constructor
    KineticString ks = new KineticString(tokens[tokenIndex++], fontSize);
    ks.parent = id;
    ks.group = tokenLoop;
  ...
  void drawShadow() {
    ...
    rotate(rot + (PI-forward));
    //victor - make shadow size relatively proportional to beast size
    scale(defaultRadius / 300, defaultRadius / 360);
    imageMode(CENTER);
    //victor - make shadow tint lighter
    tint(255, 255, 255, 90);
